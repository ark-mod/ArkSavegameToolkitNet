...
namespace ArkSavegameToolkitNet.Domain
{
    ...
    public class ArkPlayer : ArkGameDataContainerBase
    {
        ...
        public ArkPlayer(GameObject profile, GameObject player, DateTime profileSaveTime, ArkSaveData saveState) : this()
        {
            _profile = profile;
            _player = player;

            var mydata = profile.GetProperty<PropertyListStructProperty>(_myData);
            var myPlayerCharacterConfig = mydata.GetProperty<PropertyListStructProperty>(_myPlayerCharacterConfig);
            var myPersistentCharacterStats = mydata.GetProperty<PropertyListStructProperty>(_myPersistentCharacterStats);
            ...
            TribeId = mydata.GetPropertyValue<int?>(_tribeID) ?? mydata.GetPropertyValue<int?>(_tribeId);
            ...
            InventoryId = player?.GetProperty<ObjectReferenceIdProperty>(_myInventoryComponent)?.id;

            if (player?.location != null) Location = new ArkLocation(player.location.Value, saveState);
