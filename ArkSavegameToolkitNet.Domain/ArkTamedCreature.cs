...
namespace ArkSavegameToolkitNet.Domain
{
    public class ArkTamedCreature : ArkCreature
    {
        ...
        private ArkSaveData _saveState;
        ...
        public ArkTamedCreature(GameObject creature, GameObject status, GameObject inventory, ArkSaveData saveState) : this(creature, status, saveState)
        {
            //todo: inventory items not supported yet
            //var items = inventory.Where(x => x.IsItem).Select(x => x.AsItem(saveState)).ToArray();
        }

        public ArkTamedCreature(GameObject creature, GameObject status, ArkSaveData saveState) : base(creature, status, saveState)
        {
            Construct();
            ...
            DinoAncestors = ArkTamedCreatureAncestor.FromPropertyValue(creature.GetProperty<StructArrayProperty>(_dinoAncestors));
            DinoAncestorsMale = ArkTamedCreatureAncestor.FromPropertyValue(creature.GetProperty<StructArrayProperty>(_dinoAncestorsMale));

            {
                var colors = new byte[_gestationEggColorSetIndices.Length];
                var found = 0;
                for (var i = 0; i < colors.Length; i++)
                {
                    var color = creature.GetPropertyValue<byte?>(_gestationEggColorSetIndices[i]);
            ...
            {
                var basestats = new byte[_gestationEggNumberOfLevelUpPointsApplied.Length];
                var found = 0;
                for (var i = 0; i < basestats.Length; i++)
                {
                    var basestat = creature.GetPropertyValue<byte?>(_gestationEggNumberOfLevelUpPointsApplied[i]);
            ...
            NextBabyDinoAncestors = ArkTamedCreatureAncestor.FromPropertyValue(creature.GetProperty<StructArrayProperty>(_nextBabyDinoAncestors));
            NextBabyDinoAncestorsMale = ArkTamedCreatureAncestor.FromPropertyValue(creature.GetProperty<StructArrayProperty>(_nextBabyDinoAncestorsMale));
            InventoryId = creature.GetProperty<ObjectReferenceIdProperty>(_myInventoryComponent)?.id;

            TamedStats = new byte[_numberOfLevelUpPointsAppliedTamed.Length];
            UpdateStatus(status);
        }

        internal new void UpdateStatus(GameObject status)
        {
        ...
        public byte[] GestationEggColors { get; set; }
        [JsonConverter(typeof(ByteArrayConverter))]
        public byte[] GestationEggBaseStats { get; set; }
...
//"BabyCuddleWalkStartingLocation (VectorStructProperty) [*]",
...
//"DinoAncestors (StructArrayProperty) [*]",
//"DinoAncestorsMale (StructArrayProperty) [*]",
...
//"HarvestResourceLevels (ArrayProperty<int>) [*]",
...
//"NextBabyDinoAncestors (StructArrayProperty) [*]",
//"NextBabyDinoAncestorsMale (StructArrayProperty) [*]",
...
//"SaddleStructures (StructArrayProperty) [*]",
