...
namespace ArkSavegameToolkitNet.Domain
{
    public class ArkTribe : ArkGameDataContainerBase
    {
        ...
        public ArkTribe(GameObject tribe, DateTime tribeSaveTime) : this()
        {
            _tribe = tribe;

            var tribeData = tribe.GetProperty<PropertyListStructProperty>(_tribeData);
            Id = tribeData.GetPropertyValue<int?>(_tribeID) ?? tribeData.GetPropertyValue<int?>(_tribeId) ?? throw new ApplicationException("No tribe id");
            Name = tribeData.GetPropertyValue<string>(_tribeName);
            OwnerPlayerId = tribeData.GetPropertyValue<int>(_ownerPlayerDataID);
            MemberIds = tribeData.GetProperty<ArrayProperty<int>>(_membersPlayerDataID)?.values.ToArray() ?? new int[] {};
            AdminIds = tribeData.GetProperty<ArrayProperty<int>>(_tribeAdmins)?.values.ToArray() ?? new int[] { };
            MemberNames = tribeData.GetProperty<ArrayProperty<string>>(_membersPlayerName)?.values.ToArray() ?? new string[] { };
            Logs = tribeData.GetProperty<ArrayProperty<string>>(_tribeLog)?.values?.ToArray();
